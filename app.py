...
st.set_page_config(page_title="力力的坐标工具 v19.0", page_icon="📍", layout="centered")

# --- 通用工具函数 (所有模式共享) ---
def to_wgs84(v1, v2, cm, swap):
    x, y = (v2, v1) if swap else (v1, v2)
    # 简单的容错：如果x坐标明显小于y（在投影坐标里），可能弄反了
...
def generate_kmz(df, coord_mode, cm=0):
    ...
    for i, row in df.iterrows():
        try:
            # 兼容不同列名
            v1 = float(row.get("纬度/X", row.get("Lat", 0)))
            v2 = float(row.get("经度/Y", row.get("Lon", 0)))
...
if app_mode == "🖐️ 手动输入":
    ...
    with col1:
        coord_mode = st.selectbox("坐标格式", ["Decimal", "DMS", "DDM", "CGCS2000"], 
                                  format_func=lambda x: f"{x} (手动填数字)")
    ...
    st.caption("点击最后一行可以直接添加新数据。")
    
    # 初始化空表格
    if 'manual_df' not in st.session_state:
        st.session_state.manual_df = pd.DataFrame([
            {"编号": "T1", "纬度/X": 0.0, "经度/Y": 0.0},
            {"编号": "T2", "纬度/X": 0.0, "经度/Y": 0.0},
        ])
